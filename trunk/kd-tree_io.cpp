...
std::istream & operator >> (std::istream &in, kd_tree<T, D, S> &kdtree) {
	...
	if(!in.good()) throw std::runtime_error("error reading header data");

	// Check header
	if(strcmp(header, kdtree.file_header)) throw std::runtime_error("invalid kd-tree header");

	// Read file version
	unsigned short version[2];
	in.read(reinterpret_cast<char *>(version), 2 * sizeof(unsigned short));
	if(!in.good()) throw std::runtime_error("error reading version data");

	// Check supported file versions
	if(version[0] != 1 || version[1] != 0) throw std::runtime_error("unsupported kd-tree file version");

	// Read type name length
	unsigned short name_length;
	in.read(reinterpret_cast<char *>(&name_length), sizeof(unsigned short));
	if(!in.good()) throw std::runtime_error("error reading type length data");

	// Read type name
	char type_name[name_length + 1];
	in.read(type_name, name_length);
	if(!in.good()) throw std::runtime_error("error reading type data");
	...
	if(!in.good()) throw std::runtime_error("error reading dimensionality data");
	...
	if(!in.good()) throw std::runtime_error("error reading number of elements");
	if(kdtree.num_elements == 0) throw std::runtime_error("invalid number of elements in the tree");
	...
	if(!in.good()) throw std::runtime_error("error reading kd-tree data");
	...
	if(in.fail() || signature != kdtree.signature) throw std::runtime_error("error reading kd-tree signature");
...
kd_tree<T, D, S>::kd_node::kd_node(std::istream &in) {
	...
	if(!in.good()) throw std::runtime_error("error reading node data");
	
	// Process the left branch or leaf
	if(is_leaf & left_bit) left_leaf = new leaf_type(in);
	else left_branch = new node_type(in);

	// Process the right branch or leaf
	if(is_leaf & right_bit) right_leaf = new leaf_type(in);
	else right_branch = new node_type(in);
}
...
kd_tree<T, D, S>::kd_leaf::kd_leaf(std::istream &in) {
	...
	if(!in.good()) throw std::runtime_error("error reading leaf node data");
}
...
bool kd_tree<T, D, S>::kd_node::write_to_binary_stream(std::ostream &out) {
	...
	// Process the left branch or leaf
	bool retval;
	if(is_leaf & left_bit) retval = left_leaf->write_to_binary_stream(out);
	else retval = left_branch->write_to_binary_stream(out);
	if(!retval) return false;

	// Process the right branch or leaf
	if(is_leaf & right_bit) retval = right_leaf->write_to_binary_stream(out);
	else retval = right_branch->write_to_binary_stream(out);
	if(!retval) return false;
