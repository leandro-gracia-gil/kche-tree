...
int main(int argc, char *argv[]) {
  ...
  // Prepare the Mahalanobis metric. Will calculate the inverse covariance of the provided dataset.
  MahalanobisMetric<float, D> mahalanobis(train_set);
  ...
  for (unsigned int i=0; i<test_set.size(); ++i) {
    ...
    kdtree.knn<KVector>(test_set[i], K, neighbours, mahalanobis);
    #else
    kdtree.knn(test_set[i], K, neighbours, mahalanobis);
    #endif
